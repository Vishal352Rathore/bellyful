...
const SubCategory = () => {
  ...
  const { data, loading, error } = useApi(
    location.pathname === "/Groceries/category/subcategory" ||
      location.state.isSubcategory
      ? `${process.env.REACT_APP_GET_PRODUCT_BY_SUBCATEGORY}?sub_category_name=${categoryName}&page=${page}`
      : `${process.env.REACT_APP_GET_PRODUCT_BY_CATEGORY}?category_name=${categoryName}&page=${page}`,
    "GET",
    null,
    token
  );
